...
## Theory
Analysis compares ordered note-on events to reviewed overlay steps and stores both aggregate and step-level evidence. This slice uses a synthetic one-step-per-second timing model until manual input defines the real timing basis.

## Architecture Role
This source coordinates catalog, overlay, and SQLite boundaries without owning persistent state.

## Implementation Contract
It refuses variants without overlay coverage, computes wrong/missed/extra/late/early metrics, counts each weak step once, and persists results.
